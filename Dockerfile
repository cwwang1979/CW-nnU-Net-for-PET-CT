...
RUN mkdir -p /opt/app /input /output /output/images/automated-petct-lesion-segmentation && \
    ...
    chown -R user:user /opt/app /input /output /opt/algorithm/nnUNet_raw_data_base/nnUNet_raw_data
...
COPY --chown=user:user ./nnunetv2/ /opt/algorithm/nnunet/nnunetv2/
